...
export default function CustomerSupportIndex({ tickets, filters, categories, statuses, priorities }: Props) {
    ...
    useEffect(() => {
        const delay = setTimeout(() => {
            if (searchTerm !== filters.search) {
                router.get('/dashboard/customer-support', buildParams(), { preserveState: true, preserveScroll: true });
            }
        }, 300);
    ...
    const handleStatusChange = (value: string) => {
        ...
        router.get('/dashboard/customer-support', buildParams({ status: value !== 'all' ? value : undefined }), { preserveState: true, preserveScroll: true });
    };

    const handlePriorityChange = (value: string) => {
        setPriorityFilter(value);
        router.get('/dashboard/customer-support', buildParams({ priority: value !== 'all' ? value : undefined }), { preserveState: true, preserveScroll: true });
    };

    const handleCategoryChange = (value: string) => {
        setCategoryFilter(value);
        router.get('/dashboard/customer-support', buildParams({ category: value !== 'all' ? value : undefined }), { preserveState: true, preserveScroll: true });
    };

    const handlePageChange = (page: number) => {
        router.get('/dashboard/customer-support', buildParams({ page }), { preserveState: true, preserveScroll: true });
    };
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                    <Headphones style={{ width: 20, height: 20, color: '#2563eb' }} />
                                    <Box>
                                        <CardTitle>Customer Support</CardTitle>
                                        <CardDescription>
                                            Tickets raised by reps checking in on customers and vendors
                                        </CardDescription>
                                    </Box>
                                </Box>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                    <Chip label={`${tickets.total} total`} size="small" variant="outlined" />
                                    <Button onClick={() => setCreateOpen(true)} size="sm">
                                        <Plus style={{ marginRight: 4, width: 16, height: 16 }} /> New Ticket
                                    </Button>
                                </Box>
                            </Box>
                    ...
                    <CardContent>
                        {tickets.data.length === 0 ? (
                            <Box sx={{ py: 6, textAlign: 'center', color: 'text.secondary' }}>
                                <Headphones style={{ margin: '0 auto 8px', width: 32, height: 32, opacity: 0.4 }} />
                                    ...
                                    <Box component="table" sx={{ width: '100%' }}>
                                        <thead>
                                            <Box component="tr" sx={{ borderBottom: 1, borderColor: 'divider' }}>
                                                <Box component="th" sx={{ p: 1, textAlign: 'left', fontSize: '0.875rem', fontWeight: 500 }}>Ticket #</Box>
                                                <Box component="th" sx={{ p: 1, textAlign: 'left', fontSize: '0.875rem', fontWeight: 500 }}>Subject</Box>
                                                <Box component="th" sx={{ p: 1, textAlign: 'left', fontSize: '0.875rem', fontWeight: 500 }}>Contact</Box>
                                                <Box component="th" sx={{ p: 1, textAlign: 'left', fontSize: '0.875rem', fontWeight: 500 }}>Category</Box>
                                                <Box component="th" sx={{ p: 1, textAlign: 'left', fontSize: '0.875rem', fontWeight: 500 }}>Priority</Box>
                                                <Box component="th" sx={{ p: 1, textAlign: 'left', fontSize: '0.875rem', fontWeight: 500 }}>Status</Box>
                                                <Box component="th" sx={{ p: 1, textAlign: 'left', fontSize: '0.875rem', fontWeight: 500 }}>Assignee</Box>
                                        ...
                                        <tbody>
                                            {tickets.data.map((ticket) => (
                                                ...
                                                >
                                                    <Box component="td" sx={{ p: 1, fontSize: '0.875rem', fontFamily: 'monospace' }}>{ticket.ticket_number}</Box>
                                                    <Box component="td" sx={{ p: 1, fontSize: '0.875rem', fontWeight: 500 }}>{ticket.subject}</Box>
                                                    <Box component="td" sx={{ p: 1 }}>
                                                        {ticket.user ? (
                                                            <>
                                                                <Box sx={{ fontSize: '0.875rem', fontWeight: 500 }}>{ticket.user.name}</Box>
                                                                <Box sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>{ticket.user.email}</Box>
                                                            </>
                                                        ) : (
                                                            <>
                                                                <Box sx={{ fontSize: '0.875rem', fontWeight: 500 }}>{ticket.contact_name ?? '\u2014'}</Box>
                                                                <Box sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>{ticket.contact_phone ?? ''}</Box>
                                                            </>
                                                        )}
                                                    </Box>
                                                    <Box component="td" sx={{ p: 1, fontSize: '0.875rem' }}>{formatCategory(ticket.category)}</Box>
                                                    <Box component="td" sx={{ p: 1 }}>
                                                        <Chip
                                                    ...
                                                    <Box component="td" sx={{ p: 1, fontSize: '0.875rem' }}>{ticket.assignee?.name ?? '\u2014'}</Box>
                                                    <Box component="td" sx={{ p: 1, fontSize: '0.875rem', color: 'text.secondary' }}>
                                                        {ticket.created_at ? new Date(ticket.created_at).toLocaleDateString() : '\u2014'}
                                                    </Box>
                                                    <Box component="td" sx={{ p: 1 }}>
                                                        <Button asChild variant="ghost" size="sm">
                                                            <Link href={`/dashboard/customer-support/${ticket.id}`}>
                ...
                <Dialog open={createOpen} onOpenChange={setCreateOpen}>
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Open a new case file for a client. You can log the first interaction or send an SMS right after.
                            </DialogDescription>
                        </DialogHeader>
                            ...
                            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1 }}>
                                <Box>
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => createForm.setData('contact_phone', e.target.value)}
                                ...
                                <Box>
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => createForm.setData('contact_email', e.target.value)}
                            ...
                            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1 }}>
                                <Box>
                                    ...
                                    <Select value={createForm.data.category} onValueChange={(v) => createForm.setData('category', v)}>
                                        <SelectTrigger id="create_category"><SelectValue placeholder="Pick a category" /></SelectTrigger>
                                        <SelectContent>
                                            {categories.map((c) => (
                                                <SelectItem key={c.value} value={c.value}>{c.label}</SelectItem>
                                            ))}
                                ...
                                <Box>
                                    ...
                                    <Select value={createForm.data.priority} onValueChange={(v) => createForm.setData('priority', v)}>
                                        <SelectTrigger id="create_priority"><SelectValue /></SelectTrigger>
                                        <SelectContent>
                                            {priorities.map((p) => (
                                                <SelectItem key={p.value} value={p.value}>{p.label}</SelectItem>
                                            ))}
